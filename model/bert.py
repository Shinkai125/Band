    ...
    for i in range(num_layers):
        if i == 0:
            sequence_output = TransformerBlock(embed_dim, num_heads, feed_forward_dim,
                                               name='TransformerBlock-%s' % i)(embedding)
        else:
            sequence_output = TransformerBlock(embed_dim, num_heads, feed_forward_dim,
                                               name='TransformerBlock-%s' % i)(sequence_output)
...
class BERT_Model(object):
    ...
    def __init__(self, pretrained_model_dir, config_file='bert_config.json', vocab_file="vocab.txt"):
    ...
    def from_pretrained(self):
        model = load_model_from_pretrained_model(model=self.bert,
