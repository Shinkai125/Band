...
class TokenAndPositionEmbedding(layers.Layer):
    def __init__(self, maxlen, vocab_size, embed_dim):
        ...
        self.token_emb = layers.Embedding(
            input_dim=vocab_size, output_dim=embed_dim, name="word_embeddings"
        )
        self.pos_emb = layers.Embedding(
            input_dim=maxlen, output_dim=embed_dim, name="position_embeddings"
        )
...
class BERTEmbedding(layers.Layer):
    def __init__(self, maxlen, vocab_size, embed_dim, dropout_rate=0.1):
        ...
        self.segment_emb = layers.Embedding(
            input_dim=2, output_dim=embed_dim, name="segment_embeddings"
        )
        self.pos_emb = layers.Embedding(
            input_dim=maxlen, output_dim=embed_dim, name="position_embeddings"
        )
...
if __name__ == "__main__":
    ...
    embedding = BERTEmbedding(maxlen=200, vocab_size=21128, embed_dim=768)(
        [input1, input2]
    )
